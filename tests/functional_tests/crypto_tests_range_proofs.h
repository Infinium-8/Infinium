...
TEST(bpp, basics)
{
  ...
  */
  ...
  LOG_PRINT_L0("Infinium G =  " << c_point_G << " = { " << c_point_G.to_hex_comma_separated_bytes_str() << " }");
  LOG_PRINT_L0("Infinium H =  " << H << " = { " << H.to_hex_comma_separated_uint64_str() << " }");
  LOG_PRINT_L0("Infinium H2 = " << H2 << " = { " << H2.to_hex_comma_separated_uint64_str() << " }");
  ...
  bool r = bpp_gen<bpp_crypto_trait_infinium<>>(values, masks, bpp_sig, commitments, &err);
...
TEST(bpp, two)
{
...

  {
    ...
    r = bpp_gen<bpp_crypto_trait_infinium<>>(values, masks, bpp_sig, commitments, &err);
    ASSERT_TRUE(r);
  ...
  r = bpp_verify<bpp_crypto_trait_infinium<>>(sigs, &err);
  ASSERT_TRUE(r);
...
TEST(bpp, power_256)
{
  ...
  r = bpp_verify<bpp_crypto_trait_infinium<>>(sig_it_refs, &err);
  ASSERT_TRUE(r);
...
TEST(bppe, basics)
{
  ...
  */
  ...
  bool r = bppe_gen<bpp_crypto_trait_infinium<>>(values, masks, masks_2, bppe_sig, commitments, &err);
...
TEST(bppe, two)
{
...

  {
    ...
    r = bppe_gen<bpp_crypto_trait_infinium<>>(values, masks, masks2, bppe_sig, commitments, &err);
    ASSERT_TRUE(r);
  ...
  r = bppe_verify<bpp_crypto_trait_infinium<>>(sigs, &err);
  ASSERT_TRUE(r);
