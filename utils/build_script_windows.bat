...
SET ACHIVE_NAME_PREFIX=infinium-win-x64-
SET MY_PATH=%~dp0
...
msbuild src/infinium.vcxproj /p:SubSystem="WINDOWS,5.02" /p:Configuration=Release /t:Build
...
copy /Y infinium.exe bunch
copy /Y infiniumd.exe bunch
copy /Y simplewallet.exe bunch
copy /Y *.pdb bunch

%QT_MSVC_PATH%\bin\windeployqt.exe bunch\infinium.exe
...
pscp -load infinium_build_server %installer_path% build.infinium.space:/var/www/html/builds
IF %ERRORLEVEL% NEQ 0 (
...
pscp -load infinium_build_server %build_zip_path% build.infinium.space:/var/www/html/builds
IF %ERRORLEVEL% NEQ 0 (
...
set mail_msg="New %build_prefix% %TESTNET_LABEL%build for win-x64:<br>INST: https://build.infinium.space/builds/%installer_file% <br>sha256: %installer_checksum%<br><br>ZIP:  https://build.infinium.space/builds/%build_zip_filename% <br>sha256: %build_zip_checksum%<br>"

echo %mail_msg%

senditquiet.exe  -t %emails% -subject "infinium win-x64 %build_prefix% %TESTNET_LABEL%build %version%" -body %mail_msg%
