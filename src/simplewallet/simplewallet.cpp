...
bool simple_wallet::validate_wrap_status(uint64_t amount)
{
  ...
  bool r = epee::net_utils::invoke_http_json_remote_command2("http://wrapped.infinium.space/api2/get_wrap_info", req, res, http_client, 10000);
  if (!r)
  {
  ...
  uint64_t infinium_needed_for_wrap = std::stoll(res.tx_cost.infinium_needed_for_erc20);
  if (amount <= infinium_needed_for_wrap)
  {
    fail_msg_writer() << "Too small amount to cover ERC20 fee. ERC20 cost is: " 
      << print_money(infinium_needed_for_wrap) << " infinium" <<
  ...
  if (amount > unwrapped_coins_left)
  {
    fail_msg_writer() << "Amount is bigger than ERC20 tokens left available: "
      << print_money(unwrapped_coins_left) << " wInfinium";
    return false;
  }
  
  success_msg_writer(false) << "You'll receive estimate " << print_money(amount - infinium_needed_for_wrap) << " wInfinium (" << print_money(infinium_needed_for_wrap)<< " Infinium will be used to cover ERC20 fee)";
...
bool simple_wallet::transfer(const std::vector<std::string> &args_)
{
  ...
  for (size_t i = 0; i < local_args.size(); i += 2) 
  {
    ...
    if (is_address_like_wrapped(local_args[i]))
    {
      ...
      success_msg_writer(false) << "This transaction will create wInfinium (\"Wrapped Infinium\") which will be sent to the specified address on the Ethereum network.";
  ...
  try
  {
    ...
    if (!m_wallet->is_watch_only())
    {
      if(wrapped_transaction)
        success_msg_writer(true) << "Money successfully sent to wInfinium custody wallet, transaction " << get_transaction_hash(tx) << ", " << get_object_blobsize(tx) << " bytes";
      else
        success_msg_writer(true) << "Money successfully sent, transaction " << get_transaction_hash(tx) << ", " << get_object_blobsize(tx) << " bytes";
    }
    else
    {
      success_msg_writer(true) << "Transaction prepared for signing and saved into \"infinium_tx_unsigned\" file, use full wallet to sign transfer and then use \"submit_transfer\" on this wallet to broadcast the transaction to the network";
    }
  }
  catch (const tools::error::daemon_busy&)
  {
...
bool simple_wallet::sweep_below(const std::vector<std::string> &args)
{
  ...
  try
  {
    ...
    std::string filename = "infinium_tx_unsigned";
